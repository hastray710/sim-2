...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/yunze/card/importSetCardInfo",
        Pstr:'',//携带参数
      },
      // 表单校验
      rules: {
  ...
  methods: {
    ...
    importTemplate() {
      let map = {};
      map.path = "/getcsv/ImportSetCard.cvs";
      map.token = getToken();
    ...
    submitFileForm() {
      ...
      if(files.length>0){
        tools.openAsk(this,'warning', "是否确认 批量修改 分组 、备注信息？", this.uploadUp,this, this.FalseFun,null);
      }else{
