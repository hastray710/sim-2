<template>
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="mainwidth" :xs="24">
        ...
        <el-table v-loading="loading" :data="cardList" @selection-change="handleSelectionChange">
          <el-table-column label="cd_id" align="center" key="cd_id" prop="cd_id" v-if="columns[0].visible" width="60"/>
          <el-table-column label="通道模板" align="center" key="cd_code" prop="cd_code" v-if="columns[0].visible"
                           width="150">
            <template slot-scope="scope">
              {{ tools.getDkeyValue(channelCodeOptions, scope.row.cd_code) }}
            </template>
          </el-table-column>
          <el-table-column label="通道名称" key="cd_name" prop="cd_name" v-if="columns[1].visible">
            <template slot-scope="scope">
              <!--              <router-link :to="'/polling/type/index/' + scope.row.cd_id" class="link-type">-->
              <!--                <span>{{ scope.row.cd_name }}</span>-->
              <!--              </router-link>-->
              <span @click="ViewDetails(scope.row)" class="link-type">{{ scope.row.cd_name }}</span>
            </template>
          </el-table-column>
          <el-table-column label="通道别名" key="cd_alias" prop="cd_alias" v-if="columns[2].visible"/>
          <el-table-column label="运营类型" align="center" v-if="columns[3].visible" :show-overflow-tooltip="true"
                           width="100">
          ...
          <el-table-column label="通道状态" align="center" key="cd_status" v-if="columns[4].visible"
                           :show-overflow-tooltip="true" width="100">
            <template slot-scope="scope">
              <span :class="scope.row.cd_status=='1'?'mygreen':'mygred'">{{
                  tools.getDkeyValue(channelStatusOptions, scope.row.cd_status)
                }}</span>
            </template>
          </el-table-column>
          <el-table-column label="是否轮训" align="center" key="cd_lunxun" v-if="columns[5].visible"
                           :show-overflow-tooltip="true" width="80">
            <template slot-scope="scope">
              <span :class="scope.row.cd_lunxun=='1'?'mygreen':'mygred'">{{
                  tools.getDkeyValue(customize_whether, scope.row.cd_lunxun)
                }}</span>
            </template>
          </el-table-column>
          <el-table-column label="卡总数" align="center" prop="cd_count" v-if="columns[6].visible"
                           :show-overflow-tooltip="true" width="150"/>

          <el-table-column label="是否正常" align="center" prop="cd_state" v-if="columns[6].visible"
                           :show-overflow-tooltip="true" width="80">
            <template slot-scope="scope">
              <span :class="scope.row.cd_state=='1'?'mygreen':'mygred'">{{
                  tools.getDkeyValue(DictionaryState, scope.row.cd_state)
                }}</span>
            </template>
          </el-table-column>
          <el-table-column label="对接信息" prop="cd_Interface" v-if="columns[6].visible"
                           :show-overflow-tooltip="true" width="120">
            <template slot-scope="scope">
              <span :class="scope.row.cd_Interface=='4'?'mygreen':'mygred'">{{
                  tools.getDkeyValue(InterfaceState, scope.row.cd_Interface)
                }}</span>
            </template>
          </el-table-column>
            ...
            <template slot-scope="scope">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-hasPermi="['yunze:cardRoute:edit']" @click.native="handleUpdate(scope.row)">修改</el-dropdown-item>
                  <el-dropdown-item v-hasPermi="['yunze:cardRoute:find']" @click.native="ViewDetails(scope.row)">详情</el-dropdown-item>
                  <el-dropdown-item v-hasPermi="['yunze:cardRoute:remove']" @click.native="handleDelete(scope.row)">删除</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="show_ds" width="80%" append-to-body>
      <el-row>
        <el-col :span="treeCol" v-if="postageVif">
          <!--资费组树结构-->
          <div class="head-container">
            <el-input
              v-model="packageName"
              aria-placeholder="请输入资费组名称"
              clearable
              size="small"
              prefix-icon="el-icon-search"
              style="margin-bottom: 20px"
          ...
          <el-checkbox v-model="postageExpand" @change="postageTreeExpand($event, 'postage')">展开/折叠</el-checkbox>
          <el-checkbox v-model="postageNodeAll" @change="postageTreeNodeAll($event, 'postage')">全选/全不选
          </el-checkbox>
          <el-checkbox v-model="postageCheckStrictly" @change="postageTreeConnect($event, 'postage')">父子联动
          </el-checkbox>

          <el-row>
            <div class="head-container">
              <el-tree :data="packagecardRoute"
        ...
        <div style="height: 35px;" v-show="showButton">

          <router-link style="float: right;" v-hasPermi="['yunze:sys:internal']"
                       :to="'/polling/type/index/' + form.cd_id">
            <el-button size="mini" type="primary" style="margin-left: 20px" @click="aChannel">
              <span class="el-icon-connection"  @click="aChannel" >通道轮训</span>
            </el-button>
          </router-link>
          <router-link style="float: right;" v-hasPermi="['yunze:sys:internal']"
                       :to="'/polling/upStream/index/' + form.cd_id">
            <el-button size="mini" type="primary" style="margin-left: 20px"   @click="aChannel" >
              <span class="el-icon-connection" @click="aChannel">查看上游集团组</span>
            </el-button>
          </router-link>
        </div>
        ...
        <el-col :span="formCol">
          <el-form ref="form" :model="form" :rules="rules" label-width="100px">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="通道模板" prop="cd_code">
              ...
              <el-col :span="8">
                <el-form-item label="运营商" prop="cd_code">
                  <el-select v-model="form.cd_operator_type" placeholder="请选择" label-width="200">
                    <el-option
            ...
            <el-divider><i class="el-icon-mobile-phone">其他选项</i></el-divider>

            <cardRouteDetails ref="cardRouteDetails" @SetObj="SetObj" :testTXT="'测试父组件件参数'"
                              :form="form"
            ></cardRouteDetails>

            <!--            <el-divider><i class="el-icon-mobile-phone">登录账号</i></el-divider>-->
            <!--            <el-row>-->
            <!--              <el-col :span="24">-->
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" icon="el-icon-thumb" @click="postageAdd" plain size="small"
                   style="margin-left: 100px">资费组 ID
        </el-button>
        <el-button v-if="cardRouteAdd" v-hasPermi="['yunze:cardRoute:add']" type="primary" @click="Add">新 增
        </el-button>
        <el-button v-if="cardRouteEdie" v-hasPermi="['yunze:cardRoute:edit']" type="primary" @click="Edie">保 存
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  created() {
    ...
    //加载 资费组名称
    if (window['packagecardRoute'] != undefined && window['packagecardRoute'] != null && window['packagecardRoute'] != '') {
      this.packagecardRoute = window['packagecardRoute'];
      if (!tools.IsArrInside(this.packagecardRoute, "package_id", "IsNull")) {
        this.packagecardRoute.push({"package_id": "IsNull", "package_agentname": "未划分资费"});
      }
    } else {
      queryPackageSimple().then(response => {
        let jsonobj = JSON.parse(tools.Decrypt(response));
        //console.log(jsonobj);
        window['packagecardRoute'] = jsonobj.Data;
        this.packagecardRoute = window['packagecardRoute'];
        if (!tools.IsArrInside(this.packagecardRoute, "package_id", "IsNull")) {
          this.packagecardRoute.push({"package_id": "IsNull", "package_agentname": "未划分资费"});
        }
    ...
    if (str != undefined &&str != null && str != '' && str.length > 0 && str!='cardRoute' && str!='polling') {
      ...
      //console.log(jsonobj)
      if (jsonobj != null) {
        if (tools.Is_null(jsonobj.cd_id)) {
          this.queryParams.type = '4';
          this.queryParams.value = jsonobj.cd_id;
        }
      }
      this.getList();
    } else {
  ...
  methods: {
    ...
    DeptNode(value, data) {
      ...
      return data.package_agentname.indexOf(value) !== -1;
    },

    // 树权限（展开/折叠）
    postageTreeExpand(value, type) {
      if (type == 'postage') {
        let deptList = this.packagecardRoute;
        for (let i = 0; i < deptList.length; i++) {
          this.$refs.postage.store.nodesMap[deptList[i].package_id].expanded = value;
        }
      }

    },
    // 通道 树权限（全选/全不选）
    postageTreeNodeAll(value, type) {
      if (type == 'postage') {
        this.$refs.postage.setCheckedNodes(value ? this.packagecardRoute : []);
      }
    },
    // 通道 树权限（父子联动）
    postageTreeConnect(value, type) {
      if (type == 'menu') {
        this.menuCheckStrictly = value ? true : false;
      } else if (type == 'postage') {
        this.$refs.postage.setCheckedNodes(false ? this.packagecardRoute : []);
    ...
    finddata(row) {
      ...
      getCardRoute(Pwd_Str).then(response => {
        ...
        if (jsonobj.code == 200) {
          ...
          if (tools.Is_null(jsonobj.Data.sync_field)){
            jsonobj.Data.sync_field= jsonobj.Data.sync_field.split(",")
          }
          this.form = jsonobj.Data;
          if(tools.Is_null(this.form.dataType)){
            this.form.dataType = ''+this.form.dataType;
          }
          if(tools.Is_null(this.form.changeNotification)){
            this.form.changeNotification = ''+this.form.changeNotification;
          }
          if(tools.Is_null(this.form.upstream)){
            this.form.upstream = ''+this.form.upstream;
          }
          this.show_ds = true;
        } else {
          if (jsonobj.deptId == "100") {
    ...
    Assigroute(row) {
      ...
      let map = {};
      map.channel_id = row.cd_id;
      // console.log(map)
      let Pwd_Str = tools.encrypt(JSON.stringify(map));
      ThroughRoute(Pwd_Str).then(response => {
        let jsonobj = JSON.parse(tools.Decrypt(response));
        // console.log(jsonobj)
        if (jsonobj.code == 200) {
          if (jsonobj.Data != null && jsonobj.Data != undefined && jsonobj.Data != '') {
            this.postageVif = true;
            this.changePostageVif();
            let Arr = [];
            for (let i = 0; i < jsonobj.Data.length; i++) {
              let obj = jsonobj.Data[i].package_id;
              // console.log(obj)
              Arr.push(obj);

            }
            this.form.package_id = Arr;//赋值
            if (Arr != null && Arr.length > 0) {
              // console.log(this.form.package_id = Arr)
              //树型默认选中 $nextTick
              // console.log(Arr)

              this.$nextTick(() => {
                if (this.$refs.postage != null && this.$refs.postage != undefined) {
                  this.$refs.postage.setCheckedKeys(this.form.package_id);
                }
              });
              this.show_ds = false;
              this.show_ds = true;
    ...
    //获取资费组树数据
    SetTree() {
      if (this.$refs.postage != undefined && this.$refs.postage != null) {
        let arr = this.$refs.postage.getCheckedKeys();//获取数据
        if (arr != undefined && arr != null && arr.length > 0) {
          // console.log(arr)
          this.form.package_id = arr;
        } else {
          if (this.$refs.postage != undefined) {
    ...
    Add() {
      ...
      if (tools.VerificationsText(_this, _this.form.cd_code, "通道编号不能为空！") == true &&
        ...
        tools.VerificationsText(_this, _this.form.cd_state, "是否正常不能为空！") == true &&
        tools.VerificationsText(_this, _this.form.cd_Interface, "请选择对接信息不能为空！") == true
      ) {

        this.SetTree();//新增获取资费组树数据
        let url = this.form.cd_addressurl;
        let reg = /(http|https):\/\/([\w.]+\/?)\S*/;
        this.form.cd_addressurl = this.form.cd_addressurl!=undefined?this.form.cd_addressurl:"";
        console.log(this.form.cd_addressurl)
