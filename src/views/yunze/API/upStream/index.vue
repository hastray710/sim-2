      ...
      <el-col :span="mainwidth" :xs="24" >
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
          <el-form-item label="时间类型" prop="year" style="margin-bottom: 10px;line-height:35px">
            ...
            >
              <el-option
                v-for="dict in upStreamTime_type"
                :key="dict.dictLabel"
...
<!--          </el-form-item>-->
          ...
          <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
              <el-button
                ...
                @click="handleExport"
                v-hasPermi="['yunze:upStream:exportData']"
              >导出
              </el-button>
            </el-col>
        ...
        <el-table v-loading="loading" :data="upStreamList" style="able-layout:fixed">
          <el-table-column   label="群组ID" align="center" prop="groupId" v-if="columns[0].visible">
            <template slot-scope="scope"  >
              <router-link style="" v-hasPermi="['yunze:sys:internal']"
                           :to="'/polling/memberGroup/index/' + scope.row.groupId">
                  <span class="link-type">{{scope.row.groupId}}</span>
              </router-link>
            </template>
          </el-table-column>

          <el-table-column  :show-overflow-tooltip="true" label="群组名称" align="center"  prop="groupName" v-if="columns[1].visible"/>
          <el-table-column   label="群组主资费id" align="center"  prop="offeringId" v-if="columns[2].visible">
<!--            <template slot-scope="scope">-->
<!--              {{tools.getDkeyValue(yunze_polling_type,scope.row.polling_type)}}-->
<!--            </template>-->
          </el-table-column>
          <el-table-column   label="群组主资费名称" align="center"  prop="offeringName" v-if="columns[3].visible" />

          <el-table-column  :show-overflow-tooltip="true" label="通道编号" align="center"  prop="channel_id" v-if="columns[4].visible" >
            <template slot-scope="scope">
...
<!--            </template>-->
          ...
          <el-table-column   label="最近一次修改时间" align="center"  prop="upd_date" v-if="columns[6].visible"/>
...
import {getUpStreamList,exportUpStream} from "../../../../api/yunze/upStream/upStream";
...
export default {
  ...
  data() {
    return {
      ...
      // 列信息
      columns: [
        { key: 0, label: `群组ID`, visible: true },
        { key: 1, label: `群组名称`, visible: true },
        { key: 2, label: `群组主资费id`, visible: true },
        { key: 3, label: `群组主资费名称`, visible: true },
        { key: 4, label: `通道编号`, visible: true },
        { key: 5, label: `创建时间`, visible: true },
        { key: 6, label: `最近一次修改时间`, visible: true },
      ],
      // 表单校验
      rules: {
      }
  ...
  created() {
    ...
    //加载查询条件
    if(window['upStream_query']!=undefined &&  window['upStream_query']!=null && window['upStream_query']!=''){
      this.upStream_query = window['upStream_query'];
    }else{
      this.getDicts("upStream_query").then(response => {
        window['upStream_query'] = response.data;
        this.upStream_query= window['upStream_query'];
      });
    }
    ...
    //加载  时间查询类型
    if(window['upStreamTime_type']!=undefined &&  window['upStreamTime_type']!=null && window['upStreamTime_type']!=''){
      this.upStreamTime_type = window['upStreamTime_type'];
    }else{
      this.getDicts("upStreamTime_type").then(response => {
        window['upStreamTime_type'] = response.data;
        this.upStreamTime_type = window['upStreamTime_type']
      });
    }
    ...
    if(tools.Is_null(channel_id)){
      this.queryParams.type= "4";
      this.queryParams.value= Number(channel_id);
  ...
  methods: {
    ...
    getList() {
      ...
      getUpStreamList(Pwd_Str).then(response => {
           let jsonobj =  JSON.parse(tools.Decrypt(response));
          console.log(jsonobj);
          if(jsonobj.code==200){
             this.upStreamList = jsonobj.Data.Data;
    ...
    handleExport() {
      ...
      this.getParams();
      let Pwd_Str = tools.encrypt(JSON.stringify(this.queryParams));
      exportUpStream(Pwd_Str).then(response => {
        let jsonobj = JSON.parse(tools.Decrypt(response));
        //console.log(jsonobj);
        if (jsonobj.code == 200) {
