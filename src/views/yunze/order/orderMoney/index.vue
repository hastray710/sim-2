...
-->
        ...
        <!--  <el-table-column
          ...
          <template slot-scope="scope">
              <el-dropdown >
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item   v-hasPermi="['yunze:order:package']"  @click.native="manualaddPackage(scope.row.iccid)">执行分润</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>-->
    ...
    <!-- 导入查询 -->
    <el-dialog :title="number.title" :visible.sync="number.open" width="400px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="number.open = false">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
import {Exportallorders, listOrder, queryPacketSimple} from "@/api/yunze/order/order";
...
export default {
  ...
  methods: {
    ...
    getPwd_Str() {
      ...
      this.number.Pstr = Pwd_Str;
    },
    /** 导入查询 按钮*/
    Importquery(){
      this.number.title = "导入查询";
    ...
    titleUp(_this) {
      ...
      if (tools.Is_null(_this.queryParams.UpType)) {
        msg += ' 导入查询 卡号类型  ' + tools.getPackageNNamekeyValue( _this.queryParams.UpType) + '  ';
      }
      msg += ' 是否确定以上操作 ？ ';
