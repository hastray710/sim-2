    ...
    <el-row :gutter="20">
      ...
      <el-col :span="mainwidth" :xs="24">
          ...
          <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
              ...
              >
                ...
                >
                  <el-option
                    v-for="dict in typeOptionsGroup"
                    :key="dict.dictValue"
                    :label="dict.dictLabel"
                    :value="dict.dictValue"
                  />
            ...
            <el-col :span="1.5">
              ...
              <el-dropdown v-if="IsUpdInfo" style="margin-left: 10px">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item v-hasPermi="['yunze:group:updArr']" @click.native="BatchUpdate">批量修改</el-dropdown-item>
                  <el-dropdown-item v-hasPermi="['yunze:group:delArr']" @click.native="BatchDelete">批量删除</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </el-col>
        ...
        <el-table v-loading="loading" :data="cardList" ref="multipleTable" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="错误代码" align="center" prop="codeOn" v-if="columns[0].visible"/>
          <el-table-column label="错误信息" align="center" prop="message" v-if="columns[1].visible"/>
          <el-table-column label="通道编码" align="center" prop="cd_code" v-if="columns[2].visible">
            <template slot-scope="scope">
              {{ tools.getDkeyValue(channelCodeOptions, scope.row.cd_code) }}
            </template>
          </el-table-column>
          <el-table-column label="创建时间" align="center" prop="create_date" v-if="columns[3].visible">

          </el-table-column>
<!--          <el-table-column label="修改时间" align="center" prop="upd_date" v-if="columns[4].visible"/>-->
          <el-table-column label="是否通知" align="center" prop="is_notice" v-if="columns[5].visible">
            <template slot-scope="scope">
              {{ tools.getDkeyValue(customize_whether, scope.row.is_notice) }}
            </template>
          </el-table-column>
          <el-table-column label="卡总数" align="center" prop="ct_sum" v-if="columns[6].visible"/>
...
<script>
import {listGroup,DeleteId,UpdateId,DeleteArr,UpdateArr} from "@/api/monitor/group";
...
export default {
  ...
  created() {
    ...
    //加载 查询条件
    if (window['typeOptionsGroup'] != undefined && window['typeOptionsGroup'] != null && window['typeOptionsGroup'] != '') {
      this.typeOptionsGroup = window['typeOptionsGroup'];
    } else {
      this.getDicts("yunze_typeOptionsGroup").then(response => {
        window['typeOptionsGroup'] = response.data;
        this.typeOptionsGroup = window['typeOptionsGroup'];
      });
    }
  ...
  methods: {
    ...
    getList() {
      ...
      //console.log(Pwd_Str);
      listGroup(Pwd_Str).then(response => {
          let jsonobj = JSON.parse(tools.Decrypt(response));
          // console.log(jsonobj);
          if (jsonobj.code == 200) {
